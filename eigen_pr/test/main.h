...
// awf:?  Some tests *should* test the release-mode behaviour, which can be different even when all asserts pass.
//#ifdef NDEBUG
//#undef NDEBUG
//#endif
...
#define EIGEN_DEFAULT_IO_FORMAT IOFormat(4, 0, "  ", "\n", "", "", "", "")
  ...
  // some memory errors.
    ...
    namespace Eigen
    {
      ...
      void eigen_assert_call(bool a, std::string a_msg);
    }
    #define eigen_assert(a) eigen_assert_call((a), std::string(EI_PP_MAKE_STRING(__FILE__) " (" EI_PP_MAKE_STRING(__LINE__) ") : " #a))
...
namespace Eigen
{
  void eigen_assert_call(bool a, std::string a_msg) 
  {
    ...
    {
      eigen_assert_list.push_back(a_msg);
    }
  }
};
...
#include <regex>
template <class T>
std::string eigen_test_nice_typename()
{
  std::string s = typeid(T).name();
  s = std::regex_replace(s, std::regex("class Eigen::SparseMatrix<double,0,int>,class Eigen::COLAMDOrdering<int> *"), "SparseMatrix");
  s = std::regex_replace(s, std::regex("class Eigen::Matrix<double,-1,-1,0,-1,-1> *"), "MatrixXd");
  s = std::regex_replace(s, std::regex("^class +"), "");
  return s;
}
