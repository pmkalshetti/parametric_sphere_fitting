...
template<typename Scalar> void test_sparseqr_scalar()
{
  ...
  DenseVector b = dA * DenseVector::Random(A.cols());
  solver.compute(A);
  ...
  SparseVector<Scalar> sb = b.sparseView();
  MatrixType sx = solver.solve(b.sparseView());
  VERIFY_IS_APPROX(A * sx, b);

  //Compare with a dense QR solver
  ColPivHouseholderQR<DenseMat> dqr(dA);
  DenseVector refX = dqr.solve(b);
