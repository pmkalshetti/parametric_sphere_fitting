...
template<typename SparseMatrixType> void sparse_basic(const SparseMatrixType& ref)
{
  ...
  Scalar s1 = internal::random<Scalar>();
  {
    ...
    // test insert (inner random)
    {
      ...
      bool call_reserve = internal::random<int>()%2 == 1;
...
//     VERIFY_IS_APPROX(m3.cwise()/refM4, refM3.cwise()/refM4);
  ...
  // test Iterator
  {
    ...
    SparseMatrixType m(5, 9);
    m.setFromTriplets(triplets.begin(), triplets.end());
    int count = 0;
    for (typename SparseMatrixType::Iterator it(m); it; ++it) {
//      std::cout << it.row() << " " << it.col() << " " << it.value() << std::endl;
      ++count;
    }
    VERIFY_IS_EQUAL(count, 3);

    {
      typename SparseMatrixType::Iterator it(m);
      VERIFY_IS_EQUAL(it.row(), StorageIndex(0));
      VERIFY_IS_EQUAL(it.col(), StorageIndex(1));
      VERIFY_IS_EQUAL(it.value(), Scalar(1.));

      ++it;
      VERIFY_IS_EQUAL(it.row(), 3);
      VERIFY_IS_EQUAL(it.col(), 2);
      VERIFY_IS_EQUAL(it.value(), 3.);
    }
  }
...
void test_sparse_basic()
{
  for(int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_1(( sparse_basic(SparseMatrix<double,ColMajor>(8, 8)) ));
    CALL_SUBTEST_1(( sparse_basic(SparseMatrix<double,RowMajor>(8, 8)) ));
