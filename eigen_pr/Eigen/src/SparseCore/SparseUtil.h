...
/** \ingroup SparseCore_Module
  ...
  */
template<typename Scalar, typename _Index = typename SparseMatrix<Scalar>::Index >
class Triplet
{
public:
  typedef _Index Index; // removed typename by pmkalshetti
...
public:
  ...
  typedef typename triplet_t::Index Index;

  /* Construct, reserving size elements. */
  TripletArray(size_t size = 0) {
    Base::reserve(size);  // prepend Base:: by pmkalshetti
  }

  /* Add triplet to the array */
  void add(Index i, Index j, Scalar v) {
    Base::push_back(triplet_t(i, j, v));  // prepend Base:: by pmkalshetti
  }

  /* Add triplet to the array if v != 0 */
  void add_if_nonzero(Index i, Index j, Scalar v) {
    if (v != 0.) Base::push_back(triplet_t(i, j, v)); // prepend Base:: by pmkalshetti
  }
};
