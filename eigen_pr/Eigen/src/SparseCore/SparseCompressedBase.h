...
class SparseCompressedBase<Derived>::Iterator {
  public:
  typedef typename Derived::Index Index;
  typedef typename Derived::Scalar Scalar;

  Iterator(const Derived& mat) :mat(mat), k(0), it(mat, k) 
  {
    // First inner iter may be empty.  
    // If so we need to push it on until it meets a nonempty row.
    if (!it) increment();
  }

  Scalar value() const { return it.value(); }
  Index row() const { return it.row(); }
  Index col() const { return it.col(); }

  operator bool() const { return it; }

  Iterator& operator++() 
  {
    ++it; // always ++it -- if it's an error for them, it's an error for us.
    if (!it)
      increment();
    return *this;
  }
...
private:
  ...
  void increment() {
    do {
      ...
      if (k == mat.outerSize())
        return;
      it = Derived::InnerIterator(mat, k);
    } while (!it);
    // Now it is actually pointing to something.
  }

  Derived const& mat;
  Index k;
  typename Derived::InnerIterator it;
};
